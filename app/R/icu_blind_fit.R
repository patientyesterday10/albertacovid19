...
generate_icu_pred_blind <- function(max_date=max(combined$date,na.rm=TRUE)){
  ...
  combined <- combined[date<=max_date,]
  ...
  df_pred_resid[,pred_resid_trans:=pred_resid*transition*ifelse(use_residual_correction,1,0),]
  ...
  ps1 <- ggplot(df_predict,aes(x=date))+
    ...
    geom_point(data=combined,aes(y=new_cases/10,color="New Cases (STL)",fill="New Cases (STL)"),alpha=0.0)+
    ...
    ggrepel::geom_label_repel(data=df_predict[date==max(combined$date),],aes(y=ICU,label=paste0(round(ICU)),colour="ICU"),nudge_x = 3,max.overlaps = Inf,direction = "x",show.legend = FALSE,size=3)+
  ...
  ps2 <- ps1 + coord_cartesian(xlim=c(max(df_predict$date,na.rm=TRUE)-30,max(df_predict$date,na.rm=TRUE)))
  #+   ggrepel::geom_label_repel(data=df_predict[model=="Full" & type=="Forecast",],aes(y=pred_corr,colour="ICU Forecast",label=paste0(strftime(date,format = "%a %b %d"),": ",round(pred_corr))),nudge_x = -15, nudge_y=10,max.overlaps = Inf,direction = "y",show.legend = FALSE,size=3)
  
  dir.create("output/icu_blind/",showWarnings = FALSE)
  
  png(file =  paste0("output/icu_blind/",max_date,"_-_icu_forecast.png"), width = 1100, height = 500, res = 100)
  print((ps1 + ps2+labs(title=NULL,subtitle=NULL))+plot_layout(guides="collect",widths = c(1,0.6)))
  dev.off()
...
combined <- data.table(read.csv(paste0("output/ab_covid_time_series.csv")),check.names = T)
combined[,date:=as.Date(date)]
dates <- seq(as.Date("2021-03-01"),max(combined$date,na.rm=TRUE),by=1)
rm(combined)
pb <- txtProgressBar(min=0,max=length(dates))
for(i in 1:length(dates)){
  generate_icu_pred_blind(max_date = dates[i])
